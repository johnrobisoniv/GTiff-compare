...
def compare(gtiff1, gtiff2):

    # Open GeoTIFF1 with gdal, gather information
    dataset1 = gdal.Open(gtiff1, GA_ReadOnly)
    ...
    dataset2 = gdal.Open(gtiff2, GA_ReadOnly)
    ...
    row_count = 0                       # set count as indices
    col_count = 0

    while row_count <= rows1 - 1:               # the key feature: our double while loop that /
        while col_count <= cols1 - 1:           # allows you to run through all the values (really cols) in a row
        ...
        col_count = 0                   # reset the col count - we need to go through all rows for each col
